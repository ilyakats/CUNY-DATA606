...
**The streak of 1 means that there were 2 shots - 1 hit followed by 1 miss. The streak length of 0 means 1 shot that missed.**
...
**The distribution of Kobe's streak lengths is unimodal, right skewed with a peak at length 0. Based on the bar plot, roughly half of all streaks had a length of 0, a third or so of streaks had a length of 1, followed by equal number of length 2 streaks and length 3 streaks with a few length 4 streaks. The longest streak is of length 4.**
...
**The number of flips that came up heads is `r table(sim_unfair_coin)['heads']`.**
...
**Shooting percentage of 45% means that the probability of a hit is 0.45 and probability of a miss is 0.55. The `sample` function needs an argument `prob` set to `c(0.45, 0.55)`. The `size` argument should also be set to `133`.**
...
**The distribution of streak lengths of the simulated shooter is unimodal, right skewed. Same as for Kobe, the peak, the typical streak length is 0. The longest streak of baskets is `r names(table(sim_streak))[nrow(table(sim_streak))]`.**
...
**Every time a simulation of the independent shooter is run, I would expect the ditribution to be very similar to the first run. Sure some counts may be slightly different due to variability, but overall the simulation should produce very similar distribution most of the time (with the exception of a rare event).**

```{r comparison, eval=TRUE}
kobe <- data.frame(c("Kobe"),table(kobe_streak))
sim <- data.frame(c("Simulation"),table(sim_streak))
colnames(kobe) <- c("shooter", "streak", "count")
colnames(sim) <- c("shooter", "streak", "count")
streaks <- rbind(kobe, sim)

ggplot(streaks,aes(x=streak, y=count, fill=factor(shooter)))+
  geom_bar(stat="identity", position="dodge")+
  scale_fill_discrete(name="Shooter",
                      labels=c("Kobe", "Simulation"))+
  xlab("Streak Length")+ylab("No of Streaks")
```
...
**The distribution of Kobe Bryant's streak lengths is very similar to the shooter simulated using the independence model (each shot is independent of the previous shot). Althought it is possible that our simulation represents a rare event, it is unlikely. We should conclude that Kobe's distribution of streak length is accurately described by the independence model and we should reject the hot hand model.**

...
***
**The data set contains 20,000 cases. Each case corresponds to one respondent to the survey. The data set contains 9 variables:**

* `genhlth` - categorical, ordinal
* `exerany` - binary
* `hlthplan` - binary
* `smoke100` - binary
* `height` - numerical, continuous converted to discrete by rounding to inches
* `weight` - numerical, continuous converted to discrete by rounding to pounds
* `wtdesire` - numerical, continuous converted to discrete by rounding to pounds
...
# Relative frequency distribution for respondents who exercised within the past month
table(cdc$exerany)/nrow(cdc)
# Number of males in the sample
table(cdc$gender)['m']
# Proportion of the sample reporting excellent health
...
***
**The mosaic plot reveals that there is a bigger proportion of men who smoked at least 100 cigarettes than women. It also reveals that the sample has more women than men.**
...
***
**The plot above shows that as the general health decreases BMI increases (median is increasing). Interquartile range also increases with decrease in general health, and IQR increase is concentrated on the upper boundary (same for upper whisker). This means that there is a higher proportion of heavier people in categories associated with bad health and their weight issues are worse.**

**Let us consider how smoking may affect BMI.**
```{r boxplot-bmi-smoke, eval=TRUE}
boxplot(bmi ~ cdc$smoke100)
```

**Going on the general idea that smokers gain weight when they quit, I thought there may be a relationship between two variables. However, based on the box plot it appears that BMI of smokers (at least 100 cigarettes) and BMI of non-smokers are distributed about the same. The only difference based on the plot is among the outliers.**
...
```{r scatterplot-weight, eval=TRUE}
plot(cdc$wtdesire ~ cdc$weight, xlab = "Weight", ylab = "Desired Weight")
```

**Based on the scatterplot it appears that the two variables are dependent. It appears that there is a subset of respondents that are happy with their weight. The rest generally want to lose some weight, but amounts vary. There is a smaller subset of respondents that want to gain weight. And a couple of suspicious outliers looking to gain significant poundage.**
...
**`wdiff` is a numerical vector. The values should be continuous, but they have been converted to discrete through rounding of original values. Observation of 0 indicates that the respondent does not intend to lose or gain weight. Negative values show number of pounds respondent would like to lose and, similarly, positive values show number of pounds respondent would like to gain.**
...
hist(wdiff, breaks = 100, xlab = "Difference between Desired and Actual Weight", main = NA)
summary(wdiff)
```

**The histogram of `wdiff` shows that it has a unimodal, left skewed distribution. It centers just below 0. Half the respondents are either happy with their weight or want to lose up to 21 pounds. So majority of respondents are either happy with their weight or want to lose just a few pounds. There are some who are more aggressive in their weight-loss plans. There are also some respondents who want to gain weight.**

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.

```{r wdiff-gender, eval=TRUE}
boxplot(wdiff ~ cdc$gender, xlab = "Gender", ylab = "Desired Weight Loss")
# Summary of desired weight loss for men
summary(wdiff[which(cdc$gender == 'm')])
# Summary of desired weight loss for women
summary(wdiff[which(cdc$gender == 'f')])
# Proportion of men happy with their current weight
length(wdiff[which(cdc$gender == 'm' & wdiff == 0)]) / nrow(cdc[cdc$gender == 'm',])
# Proportion of women happy with their current weight
length(wdiff[which(cdc$gender == 'f' & wdiff == 0)]) / nrow(cdc[cdc$gender == 'f',])
```

**Breaking down the weight difference data by gender, we can see that for both men and women 75% of respondents either want to lose weight or are happy with the current weight (3<sup>rd</sup> quartile is 0 for both genders). However, looking at proportions 32.9% of men are happy with their current weight, while only 23.6% are in the same category. Additionally, 1<sup>st</sup> quartile for men is -20 pounds and for women it is -27 pounds. The box plot for women is also spread out more. To me this indicates that more women would like to lose weight and they want to lose more pounds than men.**
...
```{r weight-stat, eval=TRUE}
wtmean <- mean(cdc$weight)
wtmean
wtsd <- sd(cdc$weight)
wtsd
nrow(cdc[abs(cdc$weight - wtmean) < wtsd, ]) / nrow(cdc)
```

**70.76% of cases are within one standard deviatio of the mean.**

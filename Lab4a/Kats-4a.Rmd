...
**The distribution is unimodal and somewhat skewed to the right, althought the skew appears to be fairly weak.**
...
**Since we are selecting random observations from the population, the distribution of the sample should be similar to the distribution of the population - unimodal, somewhat right-skewed. However, there will be differences between two samples of 50 random observations and differences between each sample and the population. Below are histograms of `samp1` and 9 additional samples.**

```{r sample-area, eval=TRUE}
hist(samp1, xlim = range(area))

par(mfrow = c(3,3))
for(i in 1:9){
   hist(sample(area, 50), main = paste("Sample", toString(i), sep = " "), 
        xlim = range(area))
   }
```
...
**There are 5000 elements in `sample_means50`. The sampling distribution is normal centered around population mean 1499.69. If we collect significantly more samples, 50000 instead of 5000, we can expect the sampling distribution to remain the same, but it will fit much better the normal distribution (although 5000 is a lot of samples and the sampling distribution already fits very closely the normal distribution).** 
...
**There are 100 elements in `sample_means_small` representing means of 100 samples of size 50 each taken from the population.**
...
**As the sample size is increased, the center remains at the true population mean, but the spread is compressed (standard deviation is smaller), meaning that there is less variation between sample mean and population mean.** 
...
```{r price-sample, eval=TRUE}
price_sample50 <- sample(price, 50)

mean(price_sample50)
```

**The best point estimate of the population mean is the sample mean, `r mean(price_sample50)`.**
...
hist(sample_means50, breaks = 20, main = "Histogram of 5000 Samples of Size 50")
```

**Same as in the above example with `area`, the sampling distribution is normal. Based on the distribition, which is centered around the true population mean, I would expect the mean home price of the population to be around $180,000.**

```{r price-mean, eval=TRUE}
mean(price)
```
...
hist(sample_means150, breaks = 20, main = "Histogram of 5000 Samples of Size 150", 
     xlim = range(sample_means50))
```

**This sampling distribution is normal, again centered around the true population mean, which I again would guess to be around $180,000. With the larger sample the distribution is more compressed, so the spread is smaller.**
...
**The distribution of larger samples has a smaller spread. If we are concerned with making estimates that are more often close to the true value, then a small spread (larger sample size) is preferred.**

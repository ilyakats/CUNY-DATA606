...
***
**There is an upward trend in the number of girls baptized with a sharp drop off between years 1640 and 1665.**
...
***
```{r plot-boysprop-vs-year, eval=TRUE}
plot(arbuthnot$year, arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls), type = "l", 
     xlab = "Year", ylab = "Proportion of Boys")
```

**Although proportion of boys jumps up and down a bit, it is generally consistent and falls approximately between 51% and 53% of all births.**
...
**The data set includes data for years `r min(present$year)` through `r max(present$year)`. There are `r nrow(present)` observations. There are `r ncol(present)` variables - ``r names(present)``.**

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?

**The maximum number of births for a single year in Arbuthnot's data is `r format(max(arbuthnot$boys + arbuthnot$girls), big.mark = ",", scientific = FALSE)` and the minimum number of births for a single year in present day data is `r format(min(present$boys + present$girls), big.mark = ",", scientific = FALSE)`, so the scale is considerably and expectedly different.**
...
```{r present-boy-girl-ratio-plot, eval=TRUE}
plot(present$year, present$boys / present$girls, type = "l",
     xlab = "Year", ylab = "Boy-to-girl Ratio")
```

**The plot clearly shows that the ratio of boys to girls is above 1 for all years in the data set. More boys were born every year, so Arbuthnot's observation holds up for the 20<sup>th</sup> century U.S.**
...
```{r year-most-birth, eval=TRUE}
present[which.max(present$boys + present$girls), 1]
```

**Most births occurred in year `r present[which.max(present$boys + present$girls), 1]`.**
